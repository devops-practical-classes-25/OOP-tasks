...
Для реализации перейдите в папку `UserProjest`, внутри которой располагаются файлы `User.cs` и `UserComparer.cs`. Нереализованные методы выбрасывают исключения, их необходимо заменить на код для выполнения задания.
